...
#include "delayed_d_star_planner/delayed_d_star_planner.hpp"
  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  planner_ = std::make_unique<DelayedDStar>(
  ...
  nav_msgs::msg::Path & plan)
{
  ...
  // make sure to resize the underlying array that DelayedDStar uses
  planner_->setNavArr(
    costmap_->getSizeInCellsX(),
    costmap_->getSizeInCellsY());
    ...
    if (getPlanFromPotential(best_pose, plan)) {
      ...
      if (use_final_approach_orientation_) {
        ...
        if (plan_size == 1) {
          ...
          // Deal with the case of DelayedDStar producing a path with two equal last poses
...
bool
DelayedDStarPlanner::getPlanFromPotential(
  const geometry_msgs::msg::Pose & goal,
  nav_msgs::msg::Path & plan)
{
  // clear the plan, just in case
  ...
  if (!worldToMap(wx, wy, mx, my)) {
    RCLCPP_WARN(
      logger_,
      "The goal sent to the delayed_d_star planner is off the global costmap."
      " Planning will always fail to this goal.");
    return false;
  }
...
double
DelayedDStarPlanner::getPointPotential(const geometry_msgs::msg::Point & world_point)
{
  unsigned int mx, my;
...
PLUGINLIB_EXPORT_CLASS(delayed_d_star_planner::DelayedDStarPlanner, nav2_core::GlobalPlanner)
